...
export default function FellowsPage() {
  ...
  const fellowsWithData = FELLOWS.map(f => {
    const apiData = fellowScores.find(fs => fs.id === f.id)
    return {
      ...f,
      hasScores: !!apiData,
      scores: apiData?.scores,
      displayName: apiData?.name || f.name,
      displayBio: apiData?.profile?.bio || f.bio,
      displayPhoto: apiData?.profile?.avatar_url || f.photo_url,
    }
  })
    ...
    <div className="min-h-screen py-20">
      ...
      {selectedFellow && (
        <div className="max-w-lg mx-auto mb-16 fade-in-up">
          <div className="glass-card p-8 text-center relative" style={{ borderColor: 'rgba(31,196,255,0.15)' }}>
            <button
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-5">
        {fellowsWithData.map(f => (
          <button
            ...
            onClick={() => {
              if (f.scores) {
                setSelectedFellow({ id: f.id, name: f.displayName, scores: f.scores })
              }
            }}
          ...
          >
            <div className="w-14 h-14 mx-auto rounded-lg overflow-hidden border-2 border-white/10 group-hover:border-neon-cyan/40 transition-colors bg-white/5 mb-3">
              {f.displayPhoto && !failedImgs.has(f.id) ? (
                <img src={f.displayPhoto} alt={f.displayName} className="w-full h-full object-cover" onError={() => onImgError(f.id)} />
              ) : (
            ...
            <h3 className="font-bold text-xs leading-tight mb-1 text-center">{f.displayName}</h3>
            {f.hasScores ? (
